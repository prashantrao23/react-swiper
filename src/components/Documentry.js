...
const Documentry = (props) => {
    ...
    const { fetchDocumentry, documentry } = context;
    const { previewTitle } = props


    useEffect(() => {
        fetchDocumentry(); // Initiate the API request
            ...
            <Swiper breakpoints={breakpoints} modules={[Navigation, Pagination, Parallax]} parallax={true} pagination={true} navigation={true} className='!pl-3 sm:!pl-6 lg:!pl-14' >
                ...
                {!documentry || !documentry.titles ? (
                    ...
                    (
                        ...
                        documentry.titles.slice(0, 20).map((title, index) => {
